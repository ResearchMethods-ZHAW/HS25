...
#' dabei zu vier Zeitpunkten ihre Wuchshöhe gemessen. Abhängigkeiten sind hier in 
#' zwei Aspekten vorhanden: einerseits wurde jedes Pflanzenindividuum mehrfach 
#' gemessen, andererseits hat es nur jeweils eine Düngervariante erhalten.
#' 
## -----------------------------------------------------------------------------
#| message: false
# Daten laden
...
#' ## Split-plot Design
#' 
#' Es wurden zufällig fünf Untersuchungsgebiete (sites) ausgewählt, in denen 
#' jeweils der gleiche Versuchsblock implementiert wurde, bestehend aus dem 
#' Management (treatment) und der Frage, ob wilde Grossherbivoren Zugang zur Fläche hatten (plot type) (Abb. 6.1). Je ein Drittel jeder Untersuchungsfläche 
#' (zufällig zugewiesen) blieb gänzlich ungenutzt(U), wurde zu Beginn kontrolliert abgebrannt und danach sich selbst überlassen (B) oder wurde jährlich gemäht (M). Innerhalb jedes Drittels wiederum wurde (zufällig zugewiesen) die Hälfte 
#' eingezäunt (F = fenced vs. O = open), um die Beweidung durch Grossherbivoren zu verhindern.
#' 
#' 
## -----------------------------------------------------------------------------
#| message: false
...
#' Modelvergleiche (z.B mit AIcc oder drop1 funktion) benutzt werden
...
## -----------------------------------------------------------------------------
#| warning: false
best_model <- get.models(allmodels, delta==0)[[1]]
...
#' sicherstellen das keine Werte <0 predicted werden (ansonsten brauchen wir GLMM  
...
#' 
#' Im folgenden Beispiel wurde bei 18 Versuchspersonen die Wirkung von zunehmenden Schlafentzug auf die Reaktionszeit (in ms) gemessen. Bei jeder Versuchsperson 
#' wurde der Effekt an den Tagen 2–9 des Versuchs je einmal gemessen (Tage 0 und 1 
#' sind die Trainingsphase und bleiben daher unberücksichtigt).
#' 
## -----------------------------------------------------------------------------
# Daten laden
library(lme4)
...
#' 
#' Wie man in der Visualisierung sehen kann, unterscheiden sich nicht nur die 
#' Intercepts (Reaktionszeit ohne Schlafmangel), sondern der Schlafmangel scheint 
#' sich auch unterschiedlich stark auf die Reaktionszeit der Probanden auszuwirken. 
#' In diesem Fall ist es daher sinnvoll, nicht nur einen Random Intercept, sondern 
#' auch einen Random Slope zu fitten.
#' 
## -----------------------------------------------------------------------------
# Fit models
...
# Model vergleich mit AICc (package "MuMIn")
AICc(lmm_1, lmm_2)
# Model mit random slope und random intercept hat tieferen AICc
...
#' 
## -----------------------------------------------------------------------------
# Model fitten. Die function "scale" standartisiert die Kontinuierliche variable Hischlänge:
