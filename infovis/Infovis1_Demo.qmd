...
In `ggplot` wird mit dem Befehl `ggplot()` initiiert. Hier wird einerseits der Datensatz festgelegt, auf dem der Plot beruht (`data = `), sowie die Variablen innerhalb des Datensatzes, die Einfluss auf den Plot ausüben (`mapping = aes()`). 
...
Nun wollen wir mit einer von *ggplot2* zur Verfügung gestellte Theme anpassen. Wir benutzen hier *theme_classic()*, weitere Themes findet ihr [hier](https://ggplot2.tidyverse.org/reference/ggtheme.html).
...
Fast alle Aspekte eines Plots kann man aber auch noch selbst anpassen. Jürgen Dengler und sein Team empfehlen folgende Anpassungen:
...
Folgendermassen kann ich den letzten Plot als PNG-Datei abspeichern:
