...
set.seed(123)
simulationOutput <- 
  simulateResiduals(fittedModel = lmm_4, plot = TRUE, n = 1000)
# Plot residuals vs. covariates des models
...
#Checkcen ob keine Daten unter 0 gefittet wurden

# Darstellung
library("ggeffects")
pred_2 <- ggpredict(lmm_4, terms = c("year", "treatment", "plot_type") ) 
...
# Model vergleich mit AICc
compare_performance(lmm_1, lmm_2, lmm_5, lmm_4)


# Model mit tieferen AICc mit REML refitten
lmm_3 <- update(lmm_2, REML = TRUE)

lmm_4 <- glmmTMB(Reaction ~ scale(Days) + (Days | Subject),
                 family = gaussian,
                 REML = TRUE,
                 data = sleepstudy_2)
